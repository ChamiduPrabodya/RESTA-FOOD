...
function calculatePromotionDiscount(subtotal, promotion) {
  ...
  const rawDiscount = discountType === "fixed" ? discountValue : (subtotalValue * discountValue) / 100;
...
function calculateCartSubtotal(cartItems) {
  return (Array.isArray(cartItems) ? cartItems : []).reduce(
    (sum, item) => sum + (Number(item && item.unitPrice) || 0) * (Number(item && item.quantity) || 0),
    0
  );
}
